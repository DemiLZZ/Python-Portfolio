...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    """
    #Nested function to keep track of the updated information
    assert who == 0 or who == 1, 'The who argument should indicate a player.'
    # BEGIN PROBLEM 7
    def difference_of_scores(score0, score1):
        if who == 0:
            ...
            if difference > previous_high and difference != 1:
                print (str(difference), "points! That's the biggest gain yet for Player 0")
            if difference < previous_high and difference > 0:
                difference = previous_high
            return announce_highest(0, difference, score0)
        if who == 1:
            ...
            if difference > previous_high and difference != 1:
                print (str(difference), "points! That's the biggest gain yet for Player 1")
            if difference < previous_high and difference > 0:
                difference = previous_high
            return announce_highest(1, difference, score1)

    return difference_of_scores
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    best_try = 1
    current_try = 1
    best_try_average = 0 #best_try_average=0 because we wanna store this value and keep updating it afterwards
    avg = make_averaged (roll_dice, num_samples)
    while current_try <= 10:
        if avg (current_try, dice) > best_try_average:
            best_try = current_try
            best_try_average = avg (current_try, dice)
        current_try += 1
    return best_try
    # END PROBLEM 9
