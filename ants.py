...
class NinjaAnt(Ant):
    ...
    # END Problem 8
    def action(self, colony):
        ninja_place = self.place
        for bee in ninja_place.bees[ : ]:
            bee.reduce_armor(self.damage)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting:
            ...
            chosen_bee = random_or_none(list_of_bees)
            if chosen_bee:
                return self.eat_bee(chosen_bee)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while my_place:
                if my_place.ant:
                    if my_place.ant not in self.doubled_list:
                        my_place.ant.damage *= 2
                        self.doubled_list += [my_place.ant]
                    if my_place.ant.container and my_place.ant.ant and my_place.ant.ant not in self.doubled_list:
                        my_place.ant.ant.damage *= 2
                        self.doubled_list += [my_place.ant.ant]
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    list_of_actions = bee.action
    def affected(colony):
        nonlocal duration
        if duration > 0:
            duration -= 1
            effect(list_of_actions)(bee, colony)
        else:
            list_of_actions(colony)
    bee.action = affected
